...
def eval():
    ...
    for level in suite:
        ...
        for case in cases:
            ...
            try:
                result = subprocess.run([EXE_PATH, "-o", TEST_PATH + ".ll", "-emit-llvm", "-mem2reg", "-gvn", "-const-prop", "-loop-inv-hoist",  TEST_PATH + ".sy"], stderr=subprocess.PIPE, timeout=8)

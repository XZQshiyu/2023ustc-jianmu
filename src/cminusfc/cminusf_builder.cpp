        ...
        else
        {
            ...
            else if (!node.is_const)
            {
                ...
                for (auto val = 0u; val < initval.size(); val++)
                {
                    ...
                    // LOG(INFO) << current_type << " type -----------";
                    // if (initval[val] == nullptr)
                    // {
                    //     if (current_type == TYPE_INT)
                    //         initval[val] = CONST_INT(0);
                    //     else
                    //         initval[val] = CONST_FP(0.0f);
                    // }
                    // else
                    // {
                    // LOG(INFO) << "succeed to enter here ------1";
                    if (current_type == TYPE_INT)
                    {
                    ...
                    // }
                    // LOG(INFO) << "store_6";
                    // LOG(INFO) << initval[val]->get_type()->print();
                    // LOG(INFO) << iter->get_type()->print();
...
Value *CminusfBuilder::visit(ASTInitVal &node)
{
    ...
    else if (!node.is_const)
    {
        ...
        // {
        //     if(val != nullptr) // LOG(INFO) << val->print();
        //     else // LOG(INFO) << 0;
        // }
    }
    else
...
Value *CminusfBuilder::visit(ASTVar &node)
{
    ...
    if (node.length == 0)
    {
        ...
        else
        {
            ...
            if (context.is_const_exp)
            {
                ...
                // LOG(INFO) << context.val.f_val << "   " << context.val.i_val;
                // LOG(DEBUG) << ret_val->get_name() << " ==============================";
            }
            // 对变量的右值调用
            else
            {
    ...
    else
    {
        ...
        if (!context.is_const_exp)
        {
            ...
            // {
            // auto *val = dim->accept(*this);
            // // LOG(INFO) << val->print() << " ==============================";
            // // 下标合法性检查
            // auto *except_bb = BasicBlock::create(module.get(), "", context.func);
            // auto *next_bb = BasicBlock::create(module.get(), "", context.func);
            // if (val->get_type()->is_float_type())
            //     val = builder->create_fptosi(val, INT32_T);

            // auto *is_neg = builder->create_icmp_lt(val, CONST_INT(0));

            // builder->create_cond_br(is_neg, except_bb, next_bb);
            // // 下标非法,当前函数返回
            // builder->set_insert_point(except_bb);
            // if (context.func->get_return_type()->is_void_type())
            //     builder->create_void_ret();
            // else if (context.func->get_return_type()->is_float_type())
            //     builder->create_ret(CONST_FP(0.));
            ...
            else
            {
                ...
                // if(temp_val->get_type()->get_pointer_element_type()->is_pointer_type()) temp_val = builder->create_load(temp_val);
                // LOG(DEBUG) << "12222222222222222222222222222";
                // // temp_val = builder->create_gep(array_load, {val});
                // LOG(INFO) << node.array_lists.size();
                // std::vector<Value *> args;
                // for(auto dim : node.array_lists){
                //     LOG(DEBUG) << "++++++++++++++++++++++++++++++++++";
                //     args.push_back(dim->accept(*this));
                ...
                // LOG(DEBUG) << temp_val->get_type()->print();
                // LOG(DEBUG) << temp_val->get_type()->get_pointer_element_type()->print();
                // LOG(DEBUG) << temp_val->get_type()->is_array_type();
                // if(temp_val->get_type()->get_pointer_element_type()->is_array_type()){
                //     args.insert(args.begin(), CONST_INT(0));
                //     temp_val = builder->create_gep(temp_val, args);
                // }
                // else{
                //     LOG(DEBUG) << 123456;

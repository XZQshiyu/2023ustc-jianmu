...
ASTNode *AST::transform_node_iter(syntax_tree_node *n)
{
    ...
    else if (_STR_EQ(n->name, "Decl"))
    {
        ...
        //这一步操作，干掉了constDecl和Var Decl这两个中间层，把这两个区别用一个成员变量保留
        auto child = n->children[0];//child is constDecl 或者 varDecl
        // Decl可以分成const 和 var 
        // 这里看孩子就是向后看了一步
        // Decl -> ConstDecl
        // ConstDefList -> ConstDef | ConstDefList ; ConstDef
        if (_STR_EQ(child->name, "ConstDecl"))//根据孩子怕那段这个是常量还是变量
        {
            // type
    ...
    else if (_STR_EQ(n->name, "AddExp"))
    {
        ...
        if (n->children_num == 3)
        {
            auto add_exp_ndoe = static_cast<ASTAddExp *>(transform_node_iter(n->children[0]));
            node->add_exp = std::shared_ptr<ASTAddExp>(add_exp_ndoe);
            if (_STR_EQ(n->children[1]->name, "+"))
            {
                node->op = OP_ADD;
            }
